      ...
      <el-table :data="dataList" style="width: 100%" border v-loading="isLoading">
        ...
        <el-table-column align="center" label="企业编号">
          <template slot-scope="scope">
            <span size="small">{{scope.row.e_eid}}</span>
          </template>
        </el-table-column>

        <el-table-column align="center" label="员工编号">
          <template slot-scope="scope">
            <span size="small">{{scope.row.e_uid}}</span>
          </template>
        </el-table-column>
    ...
    methods: {
      salaryInputSelect (entId) {
        if (entId !== undefined) {
          ...
          this.queryModel.e_uid = '00000000'
        } else {
      ...
      doQuery () {
        if (this.queryModel.e_uid === null || this.queryModel.e_uid === undefined || this.queryModel.e_uid.toString().trim().length < 1) {
          this.queryModel.e_uid = '00000000'
        }
        if (this.queryModel.e_uid !== '00000000') {
          this.queryModel.e_eid = 'jiaxin001'
        }
        if (this.queryModel.e_eid === null || this.queryModel.e_eid === undefined || this.queryModel.e_eid.toString().trim().length < 1) {
          this.queryModel.e_eid = 'jiaxin001'
        }
